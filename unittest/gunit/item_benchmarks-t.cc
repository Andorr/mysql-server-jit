...
class ItemBenchmarks : public testing::Test {
 protected:
  ...
  //     return std::pair<std::chrono::nanoseconds, double>(elapsed_time,
  //     value);
  // }
  ...
  //   Item *build_simple_tree() {
  //     Query_block *query = ::parse(
  //         &initializer,
  //         "SELECT COUNT(*) FROM posts WHERE created_at > '2021-06-08'", 0);
  //     if (query == nullptr) {
  //       return;
  //     }
  //     // return query->
  ...
  // void benchmark_item(Item* item, size_t ins_iterations, size_t
  // time_iterations) {
  ...
  //     std::cout << "\tAvg Iterations: " << avg_iterations << " after " <<
  //     ins_iterations << " runs." <<  std::endl; std::cout << "\tAvg Time: "
  //     <<
  //     std::chrono::duration_cast<std::chrono::microseconds>(avg_time).count()
  //     << "μs after " << time_iterations << " iterations." <<  std::endl;
...
TEST_F(ItemBenchmarks, ItemTreeVisualize) {
